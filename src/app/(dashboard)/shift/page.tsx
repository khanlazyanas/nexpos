...
import { Wallet, LockOpen, Lock, Loader2, IndianRupee, Clock, History, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export default function ShiftPage() {
  ...
  const handleCloseShift = async (e: React.FormEvent) => {
    ...
    try {
      // Simulated expected cash (In real app, this is fetched from orders sum during shift time)
      // For portfolio logic: Expected = Opening Cash + Example Sales Amount (or just 0 if no sales hooked yet)
      const expected = activeShift.openingCash; 
      ...
      if (res.ok) {
        toast.success("Shift Closed & Cash Reconciled!", { style: { background: '#10b981', color: '#fff', fontWeight: 'bold' }});
        setClosingCash('');
        fetchShiftData();
      } else {
      ...
      <div className="bg-white/40 backdrop-blur-xl p-8 rounded-[2rem] border border-white shadow-sm flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 font-bold mt-2 text-sm flex items-center gap-2">
            Secure your cash register at the start and end of day <Wallet size={16} className="text-emerald-500"/>
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white/80 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-rose-100 text-rose-600 rounded-xl w-fit text-[10px] font-black uppercase tracking-widest mb-6 border border-rose-200 animate-pulse">
                    <span className="w-2 h-2 rounded-full bg-rose-500"></span> Register is Open
                  </div>
                  
                  <h2 className="text-2xl font-black text-gray-900 mb-1 tracking-tight">Close Register</h2>
                  ...
                  <div className="bg-gray-50 border border-gray-200 p-4 rounded-2xl mb-8 flex justify-between items-center">
                    <span className="text-xs font-black text-gray-500 uppercase tracking-widest">Opening Cash</span>
                    <span className="text-xl font-black text-gray-900">₹{activeShift.openingCash}</span>
                  </div>
                  ...
                  <form onSubmit={handleCloseShift} className="space-y-6">
                    <div className="relative group">
                      ...
                      <input type="number" required min="0" placeholder="Enter Closing Cash" value={closingCash} onChange={(e) => setClosingCash(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:border-rose-400 focus:bg-white font-black text-lg transition-all" />
                    </div>
                    <button type="submit" disabled={isSubmitting} className="w-full bg-gray-900 hover:bg-rose-600 text-white font-black tracking-widest uppercase text-xs py-5 rounded-2xl shadow-lg transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">
                      <Lock size={16} /> {isSubmitting ? 'Closing...' : 'End Shift & Lock'}
                    </button>
                  </form>
                </div>
              </div>
          ...
          <div className="lg:col-span-2 bg-white/60 backdrop-blur-3xl rounded-[2.5rem] shadow-sm border border-white p-6 md:p-8">
            <h3 className="text-lg font-black text-gray-900 mb-6 flex items-center gap-2"><History size={20} className="text-emerald-500"/> Shift History Log</h3>
              ...
              <table className="w-full text-left min-w-[600px]">
                <thead>
                  <tr className="border-b border-gray-100 text-gray-400 text-[10px] font-black uppercase tracking-widest">
                    ...
                    <th className="pb-4">Shift Time</th>
                    <th className="pb-4">Opening</th>
                    <th className="pb-4">Closing</th>
                    <th className="pb-4">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {shiftHistory.length === 0 ? (
                    <tr><td colSpan={5} className="py-10 text-center text-sm font-bold text-gray-400">No shift history found.</td></tr>
                    ...
                    <tr key={shift._id} className="hover:bg-white/50 transition-colors">
                      ...
                      <td className="py-4 font-black text-gray-600">₹{shift.openingCash}</td>
