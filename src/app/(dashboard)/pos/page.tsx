...
export default function POSPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100dvh-100px)] lg:h-[calc(100vh-100px)] relative z-10 animate-in fade-in duration-500">
      
      <Toaster />

      {/* ================= LEFT SIDE: Terminal ================= */}
      <div className="flex-1 h-[50%] lg:h-full flex flex-col bg-white/50 backdrop-blur-3xl backdrop-saturate-200 rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-white overflow-hidden">
        
        {/* Terminal Header */}
        <div className="px-6 py-5 border-b border-white/60 bg-white/40 shrink-0 flex flex-col sm:flex-row gap-4 justify-between items-center z-10">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div>
              <h1 className="text-2xl font-black text-gray-900 tracking-tight leading-none">Terminal</h1>
              <p className="text-xs font-bold text-gray-400 mt-0.5">{filteredProducts.length} Active SKUs</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredProducts.map(product => {
                ...
                return (
                  <div key={product._id} onClick={() => !isOutOfStock && handleAddToCart(product)} className={`group relative rounded-[1.5rem] p-5 border transition-all duration-300 select-none ${isOutOfStock ? 'bg-gray-100/50 border-gray-200 opacity-60 grayscale cursor-not-allowed' : 'bg-white border-white shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(16,185,129,0.12)] hover:border-emerald-100 cursor-pointer active:scale-[0.97] hover:-translate-y-1'}`}>
                    <h3 className="font-black text-gray-800 line-clamp-2 text-sm leading-snug min-h-[2.5rem] group-hover:text-emerald-700 transition-colors">{product.name}</h3>
                    <div className="flex justify-between items-end mt-4">
                      <span className="font-black text-xl text-gray-900 tracking-tight">₹{product.price}</span>
                      <span className={`text-[10px] px-2 py-1 rounded-lg font-black uppercase tracking-wider ${isOutOfStock ? 'bg-rose-100 text-rose-600' : product.stock_quantity <= 5 ? 'bg-orange-100 text-orange-600 animate-pulse border border-orange-200' : 'bg-emerald-50 text-emerald-600 border border-emerald-100 group-hover:bg-emerald-500 group-hover:text-white transition-colors'}`}>
      ...
      <div className="w-full h-[50%] lg:h-full lg:w-[420px] xl:w-[440px] flex flex-col bg-white/70 backdrop-blur-3xl backdrop-saturate-200 rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-white overflow-hidden">
        
        {/* Ledger Header */}
        <div className="px-6 py-5 border-b border-gray-100 bg-white/60 shrink-0 flex justify-between items-center">
          <h2 className="text-xl font-black text-gray-900 tracking-tight flex items-center gap-2"><Receipt size={20} className="text-emerald-500" /> Active Ledger</h2>
        ...
        {/* Customer Identity */}
        <div className="px-6 py-4 bg-gray-50/50 border-b border-gray-100 shrink-0 flex gap-3">
          ...
          <div className="flex-1 relative group">
            ...
            <input type="text" placeholder="Mobile Number" value={customerMobile} onChange={(e)=>setCustomerMobile(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl py-2 pl-9 pr-3 text-xs font-bold text-gray-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/10 transition-all shadow-sm" />
          </div>
        </div>

        {/* Cart Item Feed */}
        <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar bg-white/30">
          {cart.length === 0 ? (
        ...
        <div className="bg-white border-t border-gray-100 shrink-0 rounded-t-3xl shadow-[0_-10px_40px_rgb(0,0,0,0.03)] z-20">
          ...
          <div className="px-6 pb-6 pt-2">
            <div className="flex justify-between items-end mb-4 px-2">
              ...
              <span className="text-4xl font-black bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent tracking-tighter">₹{finalTotal.toLocaleString()}</span>
            </div>
            
            <button onClick={handleCheckout} disabled={cart.length === 0 || isCheckingOut} className="group relative overflow-hidden w-full bg-gray-900 disabled:bg-gray-200 text-white hover:bg-emerald-600 py-5 rounded-[1.5rem] shadow-[0_10px_20px_rgb(0,0,0,0.1)] hover:shadow-[0_15px_30px_rgba(16,185,129,0.3)] active:scale-[0.98] flex justify-center items-center gap-2 transition-all duration-300 disabled:shadow-none">
              ...
              <span className="font-black text-sm tracking-widest uppercase relative z-10 disabled:text-gray-400">Process {paymentMethod} Payment</span>
            </button>
          </div>
        </div>
      </div>
