...
export default function POSPage() {
  ...
  // 🔥 ISOLATED SCROLL REF FOR CART CONTAINER
  ...
  // 🚀 Auto-scroll ONLY the cart div smoothly
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 min-h-0 lg:h-[calc(100dvh-100px)] relative z-10 animate-in fade-in duration-500 pb-10 lg:pb-0">
      ...
      <div className="w-full flex-1 min-h-0 lg:h-full lg:w-[420px] xl:w-[440px] flex flex-col bg-white/70 backdrop-blur-3xl backdrop-saturate-200 rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-white overflow-hidden">
        ...
        {/* COMPACT: Reduced Header Padding */}
        <div className="px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-100 bg-white/60 shrink-0 flex justify-between items-center">
          <h2 className="text-base sm:text-lg font-black text-gray-900 tracking-tight flex items-center gap-2"><Receipt size={18} className="text-emerald-500" /> Active Ledger</h2>
          <button onClick={handleClearAll} className="bg-rose-50 hover:bg-rose-500 hover:text-white text-rose-600 px-3 py-1 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all active:scale-95 border border-rose-100 hover:border-rose-500">Clear</button>
        </div>

        {/* COMPACT: Reduced Input Box Padding */}
        <div className="px-3 sm:px-4 pt-2 pb-1.5 bg-gray-50/50 shrink-0 flex gap-2">
          <div className="flex-1 relative group">
            <User size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-emerald-500 transition-colors" />
            <input type="text" placeholder="Customer Name" value={customerName} onChange={(e)=>setCustomerName(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg py-1.5 pl-8 pr-2 text-xs font-bold text-gray-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/10 transition-all shadow-sm" />
          </div>
          <div className="flex-1 relative group">
            <Phone size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-emerald-500 transition-colors" />
            <input type="text" placeholder="Mobile (10-digit)" maxLength={10} value={customerMobile} onChange={(e)=>setCustomerMobile(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg py-1.5 pl-8 pr-2 text-xs font-bold text-gray-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/10 transition-all shadow-sm" />
          </div>
        </div>

        {/* COMPACT: Loyalty Wallet Padding */}
        {walletBalance > 0 && (
          <div className="mx-3 sm:mx-4 mb-1.5 p-2 bg-gradient-to-r from-amber-50 to-yellow-50 border border-yellow-200 rounded-lg flex items-center justify-between shadow-inner animate-in fade-in zoom-in-95 duration-300 shrink-0">
            <div className="flex items-center gap-2">
              <div className="bg-yellow-400 p-1.5 rounded-md text-white shadow-sm"><Gift size={14} /></div>
              <div>
            ...
            <label className="relative inline-flex items-center cursor-pointer active:scale-95 transition-transform">
              ...
              <div className="w-8 h-4 bg-yellow-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-yellow-500 shadow-sm border border-yellow-300/50"></div>
            </label>
          </div>
        )}
        ...
        {/* 🔥 ULTRA DENSE LIST: Maximum Visibility */}
        <div ref={cartContainerRef} className="flex-1 min-h-0 overflow-y-auto p-2 sm:p-3 pb-8 space-y-1 custom-scrollbar bg-white/30">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-300 space-y-2">
              <div className="bg-gray-50 p-4 rounded-full border border-gray-100"><ShoppingCart size={32} strokeWidth={1.5} /></div>
              <p className="text-xs font-bold tracking-widest uppercase">Cart is Empty</p>
            </div>
          ) : (
            cart.map(item => (
              <div key={item._id} className="group flex items-center justify-between py-1.5 px-2 bg-white border border-gray-100 shadow-sm rounded-lg hover:shadow-md hover:border-emerald-100 transition-all">
                {/* Product Name & Price in one compact flex row */}
                <div className="flex flex-col flex-1 min-w-0 pr-2">
                  <h4 className="font-black text-gray-800 text-[11px] sm:text-xs line-clamp-1">{item.name}</h4>
                  <p className="font-bold text-emerald-600 text-[10px] sm:text-[11px]">₹{(item.price * item.cartQuantity).toLocaleString()}</p>
                </div>
                
                {/* Quantity Controls & Trash Action */}
                <div className="flex items-center gap-2 shrink-0">
                  <div className="flex items-center bg-gray-50 border border-gray-100 rounded-md overflow-hidden shadow-sm">
                    <button onClick={() => updateQuantity(item._id, item.cartQuantity - 1)} className="px-1.5 py-1 hover:bg-gray-200 text-gray-500 hover:text-gray-900 transition-colors"><Minus size={10} strokeWidth={3} /></button>
                    <span className="w-5 text-center font-black text-[11px] text-gray-800">{item.cartQuantity}</span>
                    <button onClick={() => { if(item.cartQuantity < item.stock_quantity) updateQuantity(item._id, item.cartQuantity + 1); else toast.error('Stock Limit Reached!'); }} className="px-1.5 py-1 hover:bg-gray-200 text-gray-500 hover:text-emerald-600 transition-colors"><Plus size={10} strokeWidth={3} /></button>
                  </div>
                  <button onClick={() => removeFromCart(item._id)} className="text-gray-300 hover:text-rose-500 transition-colors p-1 rounded hover:bg-rose-50"><Trash2 size={14} /></button>
                </div>
              </div>
            ))
          )}
        ...
        <div className="bg-white border-t border-gray-100 shrink-0 rounded-t-3xl shadow-[0_-10px_40px_rgb(0,0,0,0.03)] z-20">
          ...
          <div className="px-3 sm:px-4 py-2 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <label className="flex items-center gap-2 cursor-pointer group">
              <div className="relative flex items-center justify-center">
                ...
                <div className="w-4 h-4 bg-white border-2 border-gray-300 rounded peer-checked:bg-emerald-500 peer-checked:border-emerald-500 transition-all"></div>
                <CheckCircle2 size={12} className="absolute text-white opacity-0 peer-checked:opacity-100 transition-opacity" strokeWidth={3} />
              </div>
              <span className="text-[10px] font-black tracking-widest text-gray-500 group-hover:text-gray-800 uppercase transition-colors">Apply {storeSettings.gstPercentage || 0}% Tax</span>
            </label>
            <div className="relative group w-24">
              <Percent size={10} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-rose-400" />
              <input type="number" placeholder="Discount" value={discount || ''} onChange={(e)=>setDiscount(Number(e.target.value))} className="w-full bg-rose-50/50 border border-rose-100 rounded-lg py-1 pl-6 pr-2 outline-none text-right font-black text-rose-600 text-[11px] focus:border-rose-300 focus:ring-2 focus:ring-rose-500/10 transition-all" />
            </div>
          </div>

          <div className="px-3 sm:px-4 py-2 grid grid-cols-4 gap-2">
            {[
              ...
              <button key={method.id} onClick={()=>setPaymentMethod(method.id)} className={`flex flex-col items-center justify-center gap-1 py-2 rounded-xl border-2 transition-all active:scale-95 ${paymentMethod === method.id ? 'border-emerald-500 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-gray-100 text-gray-400 hover:border-gray-300 hover:bg-gray-50 hover:text-gray-600'}`}>
          ...
          <div className="px-3 sm:px-4 pb-3 pt-1">
            <div className="flex justify-between items-end mb-3 px-1">
              <div>
                <span className="text-gray-400 font-black uppercase tracking-widest text-[10px]">Final Total</span>
                {usePoints && pointsToRedeem > 0 && (
                  <p className="text-[9px] font-bold text-yellow-500 mt-0.5 uppercase tracking-widest animate-pulse">-₹{pointsToRedeem} Points Used</p>
                )}
              </div>
              <span className="text-2xl sm:text-3xl font-black bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent tracking-tighter leading-none">₹{finalTotal.toLocaleString()}</span>
            </div>
            
            <button onClick={handleCheckout} disabled={cart.length === 0 || isCheckingOut} className={`group relative overflow-hidden w-full text-white py-3 sm:py-3.5 rounded-2xl shadow-[0_10px_20px_rgb(0,0,0,0.1)] active:scale-[0.98] flex justify-center items-center gap-2 transition-all duration-300 disabled:shadow-none ${paymentMethod === 'Khata' ? 'bg-indigo-600 hover:bg-indigo-500 hover:shadow-[0_15px_30px_rgba(79,70,229,0.3)]' : 'bg-gray-900 hover:bg-emerald-600 hover:shadow-[0_15px_30px_rgba(16,185,129,0.3)]'} disabled:bg-gray-200`}>
              ...
              {isCheckingOut ? <Sparkles className="animate-spin relative z-10" size={16} /> : <ShieldCheck size={16} className="relative z-10" />}
              <span className="font-black text-xs tracking-widest uppercase relative z-10 disabled:text-gray-400">Process {paymentMethod}</span>
            </button>
          </div>
        </div>
      </div>
